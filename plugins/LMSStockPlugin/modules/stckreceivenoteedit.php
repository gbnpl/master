...
if (isset($_POST['receivenoteedit'])) {
	...
	if(preg_match('/^[0-9]{4}\/[0-9]{2}\/[0-9]{2}$/', $receivenoteedit['datesettlement'])) {
		...
		if(checkdate($m, $d, $y)) {
			...
			if($id > time())
				...
				$receivenoteedit['datesettlement'] = LMSST::DateChange($receivenoteedit['datesettlement']);
		} else
	...
	if(preg_match('/^[0-9]{4}\/[0-9]{2}\/[0-9]{2}$/', $receivenoteedit['datesale'])) {
		...
		if(checkdate($m, $d, $y)) {
			...
			if($id > time())
				...
				$receivenoteedit['datesale'] = LMSST::DateChange($receivenoteedit['datesale']);
		} else
	...
	if(preg_match('/^[0-9]{4}\/[0-9]{2}\/[0-9]{2}$/', $receivenoteedit['deadline'])) {
		...
		if(checkdate($m, $d, $y)) {
			$id = mktime(0, 0, 0, $m, $d, $y);
			$receivenoteedit['deadline'] = LMSST::DateChange($receivenoteedit['deadline']);
		} else
